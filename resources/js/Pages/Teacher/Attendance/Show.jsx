...
const AttendanceShow = ({ session, students, stats, flash }) => {
    ...
    const updateAttendance = (student, status) => {
        ...
        router.post(
            route("teacher.attendance.update-attendance", session.id),
            {
    ...
    const handleExtendSession = () => {
        router.post(
            route("teacher.attendance.extend-session", session.id),
            {
    ...
    const handleDeleteAttendance = (attendanceId, studentName) => {
        ...
        if (
            ...
            router.delete(
                route("teacher.attendance.delete-attendance", [
                    session.id,
                    attendanceId,
                ]),
                {
    ...
    const handleCloseSession = () => {
        if (
            ...
            post(route("teacher.attendance.close-session", session.id), {
                preserveScroll: true,
            });
        }
    };
    ...
    return (
        <TeacherLayout title={`Sesi Absensi: ${session.title}`}>
            {/* Flash message */}
            {flash?.success && (
                <div
