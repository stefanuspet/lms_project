...
import { Calendar, Clock, Teacher } from "iconsax-reactjs";

const StudentScheduleIndex = ({
    student,
    class_name,
    scheduleByDay,
    days,
    ...
    const renderCard = (entry) => {
        const content = (
            <div className="p-4 bg-white border border-gray-100 rounded-lg shadow-sm flex flex-col gap-2">
            <div className="flex items-center justify-between">
                <p className="text-sm text-gray-500">{entry.semester}</p>
                {entry.room && (
                    <div className="flex items-center gap-1">
                        {entry.type === "extracurricular" && (
            ...
            <p className="text-sm text-gray-600 flex items-center gap-2">
                <Teacher size="16" className="text-amber-500" />
                {entry.teacher_name}
            </p>
            {entry.meeting_link && (
                <a
        ...
        // Jika sesi adalah ekstrakurikuler, card bisa diklik ke detail ekskul
        if (entry.type === "extracurricular" && entry.extracurricular_id) {
            return (
                <Link
                    key={entry.id}
                    href={route(
                        "student.extracurriculars.show",
                        entry.extracurricular_id
                    )}
                    className="block hover:shadow-md transition-shadow duration-150"
                >
                    {content}
                </Link>
            );
    ...
    return (
        <StudentLayout title="Jadwal Pelajaran">
            <div className="py-6 w-full">
                ...
                <div className="bg-white rounded-xl shadow-sm p-5 mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                        <p className="text-sm text-gray-500">Halo, {student.name}</p>
...
export default StudentScheduleIndex;
